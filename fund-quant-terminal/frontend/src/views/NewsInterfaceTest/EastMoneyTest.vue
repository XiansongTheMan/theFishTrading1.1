...
import { eastmoneyTest } from "@/news/api/eastmoney";
import { typeOptions, defaultTypeVal } from "@/news/config/eastmoney";
...
function formatBeijingTime(isoStr: string | number | null | undefined): string {
  ...
  if (!s) return "—";
  // 解析 ISO 8601 如 2026-03-07T16:53:39+08:00 转为 Date 显示
...
async function handleTest() {
  ...
  loadingInstance = ElLoading.service({
    lock: true,
    text: "请求东方财富 news API...",
    background: "rgba(0,0,0,0.4)",
  });
  try {
    const res = (await eastmoneyTest({
